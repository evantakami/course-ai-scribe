...
import { BookOpen, BookOpenCheck, Copy, Download, ArrowRight, ArrowLeft } from "lucide-react";
import { SummaryStyle, SummaryLanguage, Summary } from "@/types";
...
const SummaryReport = ({ summary, summaries = [], rawContent = "" }: SummaryReportProps) => {
  const navigate = useNavigate();
  const [summaryStyle, setSummaryStyle] = useState<SummaryStyle>(summary?.style || "casual");
  const [language, setLanguage] = useState<SummaryLanguage>("chinese");
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [summaryContent, setSummaryContent] = useState<string>(
    summary?.content || "# 摘要示例\n\n这是一个示例摘要，实际内容将根据您上传的课程内容生成。摘要将包含课程的关键知识点、重要概念和核心观点。\n\n## 主要内容\n\n- **概念一**：这里是概念一的简要解释和重要性\n- **概念二**：这里是概念二的简要解释和适用场景\n- **概念三**：这里是概念三的详细说明和示例\n\n## 核心要点\n\n1. 第一个要点及其解释\n2. 第二个要点及其具体应用\n3. 第三个要点及其注意事项\n\n## 总结\n\n本课程主要讲解了上述三个核心概念，它们之间的关系是互相补充、递进的。掌握这些概念对于理解整个学科领域具有重要意义。"
  );

  useEffect(() => {
    // Load the appropriate summary if one was passed in
    if (summary) {
      setSummaryContent(summary.content);
      setSummaryStyle(summary.style);
    } else if (summaries && summaries.length > 0) {
      // Load the first summary by default
      setSummaryContent(summaries[0].content);
      setSummaryStyle(summaries[0].style);
    }
  }, [summary, summaries]);
  ...
  const handleStyleChange = (style: SummaryStyle) => {
    ...
    // Find the matching summary from summaries
    if (summaries && summaries.length > 0) {
      const selectedSummary = summaries.find(s => s.style === style);
      if (selectedSummary) {
    ...
    // If no matching summary is found, use the demo content
    setTimeout(() => {
      let newContent = "";
